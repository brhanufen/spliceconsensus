...
**Brhanu F. Znabu, Zohaib Atif**

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.20948820.svg)](https://doi.org/10.5281/zenodo.20948820)


A leakage-aware, head-to-head benchmark of three published splicing variant-effect
predictors run through the [Proto](https://proto.evodesign.org) tool ecosystem,
...
| SpliceAI | 0.819 | 0.321 | [0.294, 0.353] |
| SpliceTransformer | 0.786 | 0.317 | [0.285, 0.352] |
| SPANR (legacy) | 0.748 | 0.228 | [0.201, 0.258] |

- **Pangolin is the strongest predictor**, by a clear margin (its AP confidence interval does not overlap the others).
- **All three modern tools beat the legacy SPANR** model.
- The ranking **reproduces the ordering reported by the Pangolin authors**, a correctness check on the pipeline.
- A calibrated **consensus** of the three modern tools, evaluated on an exon-grouped held-out split, **does not meaningfully improve over Pangolin alone** (held-out AP 0.443 vs 0.442; the model essentially relies on Pangolin). For this MFASS benchmark, Pangolin is the strongest single predictor.
- **All four tools share a blind spot.** Recall of disrupting variants declines with distance from the splice site, and **22% of disrupting variants (228/1,050) are missed by every tool**, 71% of them exonic and 75% more than 10 bp from a splice site. These are consistent with exon-interior regulatory disruptions (possible exonic enhancer or silencer effects) that splice-site models cannot see.
...
0.80); beyond 20 bp, recall falls to 0.48 / 0.35 / 0.29 (Pangolin / SpliceAI / SpliceTransformer)
and 0.20 for SPANR. More than half of disrupting variants lie more than 10 bp from a
splice site, and even Pangolin detects only 59% of them. The practical message: trust
these tools near splice sites, use caution in the exon interior, where roughly one in
five disrupting variants (228/1,050, 71% exonic) is invisible to all four predictors
(`src/failure_mode.py`). Distance is computed from the MFASS construct and cross-checked
against genomic coordinates (r=0.997).
...
1. **Score** each variant with each predictor in its real hg38 context
   ...
   SPANR uses its precomputed max-tissue delta magnitude.
2. **Benchmark** against the experimental label (`src/analyze.py`, CPU): per-tool
   ...
   against the continuous readout, an exon-grouped held-out consensus, and figures.

Minus-strand alleles are normalized to the plus strand before scoring; for the
three Proto-scored modern predictors, full coverage (28,972 of 28,972) confirms no
variant is silently dropped (SPANR covers slightly fewer, a precomputed subset). SpliceAI
returns no score for variants outside an annotated gene; these are assigned 0 (a
minority, shown by the depletion of disrupting variants among zero-scored variants).
